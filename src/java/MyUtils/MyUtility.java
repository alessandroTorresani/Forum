...
public class MyUtility {
    ...
    private static final String LINK_REGEX = "((https?|ftp|file)://)?[^\\.]+(\\.[^\\.]+)+"; // controllo del link
                    ...
                    if ((second > 0) && (first < second)) {
                        ...
                        if (link.matches(LINK_REGEX)) {
                            if ((link.startsWith("http")) || (link.startsWith("ftp")) || (link.startsWith("https"))) { // se il link comincia già con http/ftp/https... non aggiungo nulla
                                substitute = "<a href = '" + link + "' target='_blank'> name </a>";
                            } else {
                                substitute = "<a href='http://" + link + "' target='_blank'> name </a>"; //altrimenti aggiungo il link
                            }
